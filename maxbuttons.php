...
function maxbuttons_set_global_paths() {
	...
	define('MAXBUTTONS_VERSION_NUM', '1.37');
...
function maxbuttons_admin_menu() {
	...
	// Now add the submenu page for the Upgrade To Pro page
...
function maxbuttons_get_published_buttons() {
	...
	return $wpdb->get_results(	$wpdb->prepare("SELECT * FROM " . maxbuttons_get_buttons_table_name() . " WHERE status <> 'trash' %s ", $order)) ;
}
